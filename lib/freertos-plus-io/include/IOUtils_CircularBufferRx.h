...
{
	SemaphoreHandle_t xNewDataSemaphore;	/* Semaphore used to indicate the presence of new data. */
	...
	TickType_t xBlockTime;			/* The amount of time a task should be held in the Blocked state (not using CPU time) to wait for data to become available when it attempts a read. */
...
void vIOUtilsSetCircularBufferTimeout( Peripheral_Control_t *pxPeripheralControl, TickType_t xMaxWaitTime );
