...
{
	SemaphoreHandle_t xWriteAccessMutex;/* Mutex used to indicate the end of transmission, meaning the buffer being transmitted is free for other use, and a new Tx can start if desired. */
	TaskHandle_t xTask;					/* Task currently holding the mutex */
...
Zero_Copy_Tx_State_t * pxZeroCopyState = pxPeripheralControl->pxTxControl->pvTransferState;									\
																															\
	if( pxZeroCopyState->xTask == xTaskGetCurrentTaskHandle() )													\
	{																														\
...
portBASE_TYPE xIOUtilsGetZeroCopyWriteMutex( Peripheral_Control_t * const pxPeripheralControl, uint32_t ulRequest, TickType_t const xMaxWaitTime );
