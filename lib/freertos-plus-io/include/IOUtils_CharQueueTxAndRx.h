...
{
	QueueHandle_t xQueue;		/* A pointer to the queue itself. */
	TickType_t xBlockTime;	/* The amount of time a task should be held in the Blocked state (not using CPU time) to wait for data to become available if a read attempt is made on an empty queue, or for space to become available when a write attempt is made on a full queue. */
...
void xIOUtilsSetTxQueueTimeout( Peripheral_Control_t * const pxPeripheralControl, const TickType_t xMaxWaitTime );
void vIOUtilsSetRxQueueTimeout( Peripheral_Control_t * const pxPeripheralControl, const TickType_t xMaxWaitTime );
...
portBASE_TYPE xIOUtilsWaitTxQueueEmpty( Peripheral_Control_t * const pxPeripheralControl, const TickType_t xMaxWaitTime );
