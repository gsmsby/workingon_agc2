...
Zero_Copy_Tx_State_t *pxZeroCopyState;
	...
	if( pxPeripheralControl->pxTxControl != NULL )
	{
		...
		if( pxZeroCopyState != NULL )
		{
			...
			if (pxZeroCopyState->xWriteAccessMutex)
...
portBASE_TYPE xIOUtilsGetZeroCopyWriteMutex( Peripheral_Control_t * const pxPeripheralControl, uint32_t ulRequest, const TickType_t xMaxWaitTime )
{
...
Zero_Copy_Tx_State_t *pxZeroCopyState;
	...
	if (xReturn == pdPASS)
		pxZeroCopyState->xTask = xTaskGetCurrentTaskHandle();
